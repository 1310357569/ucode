...
function ojbk_enc(url){
    key = md5('ojbk666');
    string = ojbkbase64_decode(url);   
    len = key.length;   
    code = '';   
    ...
    return ojbkbase64_decode(code);
}
function ojbk_random_enc(url){
    randStr=url.substring(16,24);
    string=url.substring(24,url.length);
    string = ojbkbase64_decode(url);  
    key = md5('ojbk666'+randStr); 
    len = key.length;   
    code = '';   
    for (i = 0; i < string.length; i++) {   
        k = i % len;   
...
function ojbk_decode_base64(url){
    string=url.substring(8,url.length);
    newstring=base64decode(string);
    return newstring.substring(8,newstring.length-8);
}


function ojbkbase64_decode(data) {
...
var MacPlayer = {
	...
	'Init': function() {
		...
		if (player_data.encrypt == '1') {
			...
			player_data.url = ojbk_decode_base64(player_data.url);
			player_data.url_next = ojbk_decode_base64(player_data.url_next)
		}
